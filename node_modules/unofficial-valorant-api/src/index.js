...
module.exports = {
    ...
    returndata: async function(url, type, data) {
        ...
        if(response.response) return {status: response.response.status, data: response.response.data, ratelimits: {used: Number(response.response.headers["x-ratelimit-limit"]), remaining: Number(response.response.headers["x-ratelimit-remaining"]), reset: Number(response.response.headers["x-ratelimit-reset"])}, error: null}
        return this.links.includes(type) ? {status: response.status, data: response.data, ratelimits: {used: Number(response.headers["x-ratelimit-limit"]), remaining: Number(response.headers["x-ratelimit-remaining"]), reset: Number(response.headers["x-ratelimit-reset"])}, error: null} : {status: response.status, data: response.data.data, ratelimits: {used: Number(response.headers["x-ratelimit-limit"]), remaining: Number(response.headers["x-ratelimit-remaining"]), reset: Number(response.headers["x-ratelimit-reset"])}, error: null}
    },
    /**
     * Get account details
     ...
     * @param {String} name - Name of the player
     * @param {String} tag - Tag of the player
     * @param {("e3a3" | "e3a2" | "e3a1" | "e2a3" | "e2a2" | "e2a1" | "e1a3" | "e1a2" | "e1a1")} [filter] - Optional filter for season (Only v2)
     */
    getMMR: async function(version, region, name, tag, filter)  {
        return this.returndata(`https://api.henrikdev.xyz/valorant/${version}/mmr/${region}/${encodeURI(name)}/${encodeURI(tag)}${filter != null ? `?filter=${filter}` : ""}`, "mmr")
    },
    /**
     * Get MMR History
     * @param {("eu" | "na" | "ap" | "kr")} region Cluster Region (eu, na, ap, kr)
    ...
    getMMRByPUUID: async function(version, region, puuid, filter) {
        if(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(puuid)) return this.returndata(`https://api.henrikdev.xyz/valorant/${version}/by-puuid/mmr/${region}/${puuid}${filter != null ? `?filter=${filter}` : ""}`, "mmr") 
     ...
     * @param {("escalation" | "spikerush" | "deathmatch" | "competitive" | "unrated" | "replication" | "custom" | "newmap" | "snowball")} [filter] Filter for only games with the specific type
     * @param {("haven" | "icebox" | "bind" | "split" | "ascent" | "breeze" | "fracture")} [map] Filter for only games with the specific map 
    */
    getMatches: async function(region, name, tag, size, filter, map) {
        return this.returndata(`https://api.henrikdev.xyz/valorant/v3/matches/${region}/${encodeURI(name)}/${encodeURI(tag)}${map != null && filter == null ? `?map=${map}${size != null ? `&size=${size}` : ""}` : map == null && filter != null ? `?filter=${filter}${size != null ? `&size=${size}` : ""}` : map != null && filter != null ? `?filter=${filter}&map=${map}&size=${size}` : ""}`, "matches")
    },
    /**
     ...
     * @param {("haven" | "icebox" | "bind" | "split" | "ascent" | "breeze" | "fracture")} [map] Filter for only games with the specific map
     */
    getMatchesByPUUID: async function(region, puuid, size, filter, map) {
        if(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(puuid)) return this.returndata(`https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/${region}/${puuid}${map != null && filter == null ? `?map=${map}${size != null ? `&size=${size}` : ""}` : map == null && filter != null ? `?filter=${filter}${size != null ? `&size=${size}` : ""}` : map != null && filter != null ? `?filter=${filter}&map=${map}&size=${size}` : ""}`, "matches")
        return this.returndata("puuidvalidation")
    },
    /**
     * Get Leaderboard
    ...
    getLeaderboard: async function(region, name, tag) {
        return this.returndata(`https://api.henrikdev.xyz/valorant/v1/leaderboard/${region}${name != null && tag == null ? "" : name == null && tag != null ? "" : name != null && tag != null ? `?name=${encodeURI(name)}&tag=${encodeURI(tag)}` : ""}`, "leaderboard")
    },
    /**
     * Get Status
     * @param {("eu" | "na" | "ap" | "kr")} region Cluster Region (eu, na, ap, kr)
    ...
    /**
     * Get Content
     * @param {("ar-AE" | "de-DE" | "en-GB" | "en-US" | "es-ES" | "es-MX" | "fr-FR" | "id-ID" | "it-IT" | "ja-jp" | "ko-KR" | "pl-PL" | "pt-BR" | "ru-RU" | "th-TH" | "tr-TR" | "vi-VH" | "zh-CN" | "zh-TW")} locale Translation of the content response
     */
    getContent: async function(locale) {
        return this.returndata(`https://api.henrikdev.xyz/valorant/v1/content${locale != null ? `?locale=${locale}` : ""}`, "content")
    },
    /**
     * Get Store Offers
     */
    ...
    getWebsiteContent: async function(countrycode, filter) {
        return this.returndata(`https://api.henrikdev.xyz/valorant/v1/website/${countrycode}${filter != null ? `?filter=${filter}` : ""}`, "website")
    },
    /**
     * Get Raw Source Server Response
     * @param {("competitiveupdates" | "mmr" | "matchdetails" | "matchhistory")} type Type of the request
     * @param {(String | String[])} uuid PUUID or MatchID 
     * @param {("eu" | "na" | "ap" | "kr")} region Cluster Region (eu, na, ap, kr)
